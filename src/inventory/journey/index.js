  ...
  try{
    ...
    html += '<body>'
      html += '<table padding="0" width="700px">'
        html += '<tr class="header-text"><td colspan="3">'+info.player+' '+info.nameKey
        if(info.isJourneyGuide) html += ` Journey Guide`
        if(!info.isJourneyGuide) html += ` squad`
        html += ` Gear and relic mats needed</td></tr>`
        if(info.includeGuideUnit) html += `<tr class="${bkImg}"><td colspan=3 class="gear-name">Includes needed gear/relic material for ${info.nameKey} to R5</td></tr>`
